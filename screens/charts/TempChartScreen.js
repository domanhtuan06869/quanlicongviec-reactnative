...
export default function TempChartScreen(props) {

  return (
    <View style={{flex:1,padding:10,}}>
      <Text>Today statistics</Text>
      <WebView source={{uri:'http://52.221.226.126/chart/hour/temp/1'}}/>
      <Text>This month statistics</Text>
      <WebView source={{uri:'http://52.221.226.126/chart/day/temp/1'}}/>
    </View>
  );
}

TempChartScreen.navigationOptions = props=>{
  return{
  header:(
    <View style={styles.header}>
        <Ionicons onPress={()=>{props.navigation.openDrawer()}} color={'white'} size={35} name={Platform.OS=='ios'?'ios-menu':'md-menu'}/>
        <Text style={{fontSize:20,marginTop:5,color:'white',}}>Temp Chart</Text>
